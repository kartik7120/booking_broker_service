...
func (c *Config) HandleWebhookEvents(w http.ResponseWriter, r *http.Request) {
	...
	case "payment.succeeded":
		...
		_, err = c.Payment_service.CommitIdempotentKey(context.Background(), &payment_service.CommitIdempotentKeyRequest{
			BookedSeatsIds: bookResp.BookSeatsId,
		})

		if err != nil {
			fmt.Println("error commiting booked seats in the payment.success case : ", err.Error())
			redisValue.PaymentStatus = "failed"
			redisValue.ErrorMessage = fmt.Sprintf("booking error when commiting booked seats : %v", err)
			return
		}
...
func (c *Config) Create_Customer(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Printf("idempotent key in commit idempotent key function : %s\n", requestBody.IdempotentKey)
	fmt.Printf("customer id for commit idempotent key function : %s\n", response.CustomerId)
	fmt.Printf("order id for commit idempotent key function %v\n", redisResult.OrderIDs)
	fmt.Printf("movie time slot id for commit idempotent key function %v \n", redisResult.MovieTimeSlotID)
	fmt.Printf("booked seats ids for commit idempotent key function %v \n", redisResult.BookedSeatsIDs)
